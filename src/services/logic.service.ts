  ...
  try {
    ...
    await Promise.all(
      RequestData.validation_data.map((request_details) =>
        ValidateSingleRequest(request_details)
      )
    );
...
export const FetchPeldataByClientRef = (
      ...
      if (callbackData.request_details) {
        if (callbackData.request_details.api_callback_url) {
          ...
          await callback.sendCallback(
            callbackData.request_details.api_callback_url,
            callbackResponse
          );
          ...
          throw new Error(
            " Data Missing api_callback_url / technical Error (Client should retry the request with callback url)"
          );
        }
      } else {
        throw new Error(
          " Data Missing request details / technical Error (Client should retry the request with api request details)"
        );
      }
    } else {
      throw new Error(
        " Data not found / technical Error (Client should retry the request)"
      );
    }
...
const updateCallbackSuccess = async (request_id) => {
  const result = await execute<{ affectedRows: number }>(
    PelezaQueries.UpdateCallBackSuccess,
    [request_id]
  );
  return result.affectedRows > 0;
};

const updateCallbackError = async (
  request_id,
  api_callback_error_type,
  api_callback_error_details
) => {
  const result = await execute<{ affectedRows: number }>(
    PelezaQueries.UpdateCallBackError,
    [api_callback_error_type, api_callback_error_details, request_id]
  );
...
export const clientLogin = async (LoginData: ILoginRequestData) => {
  return new Promise(async (resolve, reject) => {
    try {
      const LoginResult: IClientData = (
        await execute<IClientData>(PelezaQueries.LoginQuerry, [
      ...
      const pwdEncoded = Buffer.from(
        LoginData.client_password,
        "utf8"
      ).toString();
      const pwdInmd5 = crypto
        .createHash("md5")
        .update(pwdEncoded)
        .digest("hex");
      ...
      const CompanyResult: ICompanyData = (
        await execute<IClientData>(PelezaQueries.FetchCompany, [
          LoginData.client_company_id,
        ])
      )[0];
